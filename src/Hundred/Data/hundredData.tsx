...
const useFetchData = () => {
    ...
    useEffect(() => {
        console.log('comptrollerData data here --- ', comptrollerData);
        if (comptrollerData) {
            updateData();
        }
    }, [comptrollerData]);
    ...
    const getComptroller = async () => {
        if (network) {
            const net = { ...network };
            const comptroller = library
                ? await getComptrollerData(library, net)
                : await getComptrollerData(new ethers.providers.JsonRpcProvider(net.networkParams.rpcUrls[0]), net);
    ...
    const getData = async () => {
        ...
        if (network && comptrollerData) {
            ...
            const markets = account
                ? await fetchData({
                      allMarkets: [...comptroller.allMarkets],
                      userAddress: account,
                    ...
                    oldGauges.forEach((g) => {
                        if (+g.userStakeBalance.toString() > 0) {
                            const market = markets.markets.find(
                                (m) => m.pTokenAddress.toLowerCase() === g.generalData.lpToken.toLowerCase(),
                            );
                            if (market) {
                                const temp = {
        ...
        try {
            ...
            if (!firstLoad.current)
                timeoutId.current = setTimeout(
                    updateData,
                    (errorsCount.current < 2 ? errorsCount.current + 1 : errorsCount.current) * 10000 + 10000,
                );
            else {
                if (errorsCount.current < 2)
                    timeoutId.current = setTimeout(updateData, (errorsCount.current + 1) * 1000);
                else if (errorsCount.current === 3) timeoutId.current = setTimeout(updateData, 5000);
                else if (errorsCount.current === 7) {
                    //if(firstLoad.current) setSpinnerVisible(false)
                    toastErrorMessage(
                        `${error?.message.replace('.', '')} on Page Load\n${
                            error?.data?.message
                        }\nPlease refresh the page after a few minutes.`,
                    );
                } else timeoutId.current = setTimeout(updateData, 10000);
            }
            update.current = false;
    ...
    const getMaxAmount = async (market: CTokenInfo, func?: string): Promise<BigNumber> => {
        ...
        if (m.isNativeToken && library) {
            ...
            if (func === 'repay' && library) {
                ...
                const balance = m.underlying.walletBalance.gt(BigNumber.from('0'))
                    ? m.underlying.walletBalance.subSafe(gasRemainder)
                    : m.underlying.walletBalance;
    ...
    const getMaxRepayAmount = async (market: CTokenInfo): Promise<BigNumber> => {
        if (marketsData) {
            ...
            if (m) {
                ...
                const amount = BigNumber.parseValueSafe(
                    borrowBalanceInTokenUnit.mulSafe(maxRepayFactor).toString(),
                    m.underlying.decimals,
                );
    ...
    const updateMarket = async (
        market: CTokenInfo | GaugeV4 | null,
    ...
    const handleUpdateClaimHndLegacy = async (chain: number) => {
        if (network && networkId.current === chain && account && library && marketsData && comptrollerData) {
            ...
            const blockProvider: ethers.providers.Web3Provider = net.blockRpc
                ? new ethers.providers.JsonRpcProvider(net.blockRpc)
                : library;
            ...
            resChunks.forEach((x: any, index: number) => {
                ...
                if (+totalSupply > 0) {
                    const newSupplyIndex =
                        ...
                        ((blockNum - compSupplyState.block) * +compSpeeds * 1e36) / +totalSupply;
        ...
        if (network && networkId.current === chain && account && library) {
            ...
            if (isEnterMarket === shouldReturn && marketsData) {
                ...
                const m = markets.find(
                    (x) => x.underlying.symbol.toLowerCase() === market.underlying.symbol.toLowerCase(),
                );
            ...
            if (+supplyBalanceInTokenUnit.toString() > +market.supplyBalanceInTokenUnit.toString()) {
                ...
                if (m !== undefined && networkId.current === chain && comptrollerData) {
                    ...
                    if (market.isNativeToken) {
                        const [price] = await comptroller.ethcallProvider.all([
                    ...
                    if (+walletBalance.toString() < +market.underlying.walletBalance) {
                        const supplyBalance = BigNumber.parseValue(
                            (+supplyBalanceInTokenUnit.toString() * +underlyingPrice.toString()).noExponents(),
                        );
    ...
    const handleUpdateBorrow = async (market: CTokenInfo, chain: number) => {
        if (network && networkId.current === chain && account && library && comptrollerData) {
            ...
            const calls = [
                ctoken.getAccountSnapshot(account),
                comptroller.oracle.getUnderlyingPrice(market.pTokenAddress),
            ];
            if (!market.isNativeToken) {
                const tokenContract = new Contract(market.underlying.address, TOKEN_ABI);
            ...
            const wallet = market.isNativeToken ? await library.getBalance(account) : res[2];
            const walletBalance = BigNumber.from(wallet, market.underlying.decimals);

            if (
                +borrowBalanceInTokenUnit.toString() > +market.borrowBalanceInTokenUnit.toString() &&
    ...
    const handleUpdateBackstopDeposit = async (market: CTokenInfo, chain: number) => {
        if (network && networkId.current === chain && account && library && market.backstop && comptrollerData) {
            const comptroller = { ...comptrollerData };
            const net = { ...network };
            const bstop = comptroller.backstopPools.find(
                (x) => x.underlyingTokens.toLowerCase() === market.underlying.address.toLowerCase(),
            );

            if (bstop && net.backstopMasterChef) {
                const backstopAbi =
                    net.backstopMasterChef.version === MasterChefVersion.v1
                        ? BACKSTOP_MASTERCHEF_ABI
                        : BACKSTOP_MASTERCHEF_ABI_V2;
                ...
                if (market.isNativeToken) {
                    const [balance] = await comptroller.ethcallProvider.all([
                ...
                if (
                    +value.toString() > +market.backstop.userBalance.toString() &&
                    +walletBalance.toString() < +market.underlying.walletBalance
                ) {
                    console.log('Should Return', value.toString());
                    const markets = [...marketsData];
                    const m = markets.find(
                        (x) => x.underlying.address.toLowerCase() === market.underlying.address.toLowerCase(),
                    );
                    if (m && m.backstop) {
    ...
    const handleUpdateBackstopClaim = async (market: CTokenInfo, chain: number) => {
        if (network && networkId.current === chain && account && library && comptrollerData) {
            ...
            if (backstopMasterChef && backstopMasterChef.address && market.backstop) {
                const backstopAbi =
                    backstopMasterChef.version === MasterChefVersion.v1
                        ? BACKSTOP_MASTERCHEF_ABI
                        : BACKSTOP_MASTERCHEF_ABI_V2;
                ...
                if (
                    +hundredBalance.toString() > +hndBalance.toString() &&
                    +pendingHnd.toString() < +market.backstop.pendingHundred.toString()
                ) {
        ...
        if (network && networkId.current === chain && account && library) {
            const tokenContract = gauge.generalData.gaugeHelper
                ? gauge.generalData.backstopGauge
                    ? new ethers.Contract(
                          gauge.generalData.lpBackstopTokenUnderlying
                              ? gauge.generalData.lpBackstopTokenUnderlying
                              : '',
                          CTOKEN_ABI,
                          library,
            ...
            if (+value.toString() > +gauge.userAllowance.toString()) {
                ...
                const gauges = [...gaugesV4Data];
                const g = gauges.find(
                    (x) => x.generalData.address.toLowerCase() === gauge.generalData.address.toLowerCase(),
                );
        ...
        if (network && networkId.current === chain && library && account) {
            if (gaugeV4 && comptrollerData) {
                ...
                const contract = net.isMinterV2
                    ? new Contract(gaugeV4.generalData.address, GAUGE_V5_ABI)
                ...
                const [balance, value, claimable_reward] = await comptroller.ethcallProvider.all([
                    balanceContract.balanceOf(account),
                    net.isMinterV2
                        ? contract.claimable_tokens(account, net.hundredAddress)
                ...
                if (
                    +claimable.toString() < +gaugeV4.userClaimableHnd.toString() &&
                    +hundred.toString() > +hndBalance.toString()
                ) {
                    console.log('Should Return', claimable.toString());
                    const gauges = [...gaugesV4Data];
                    const g = gauges.find(
                        (x) => x.generalData.address.toLowerCase() === gaugeV4.generalData.address.toLowerCase(),
                    );
    ...
    const handleUpdateClaimRewards = async (gaugeV4: GaugeV4, chain: number) => {
        if (network && networkId.current === chain && library && account) {
            if (gaugeV4) {
                ...
                if (
                    +gaugeV4.claimable_reward.toString() > 0 &&
                    +claimableReward.toString() < +gaugeV4.claimable_reward.toString()
                ) {
        ...
        if (network && networkId.current == chain && library && account && gaugeV4) {
            ...
            if (+userStaked.toString() > +gaugeV4.userStakeBalance.toString()) {
                ...
                const market = gauge?.generalData.backstopGauge
                    ? markets.find(
                          (x) => x.pTokenAddress.toLowerCase() === gauge.generalData.lpTokenUnderlying.toLowerCase(),
                      )
                    : markets.find(
                          (x) =>
                              x.underlying.address.toLowerCase() ===
                              gaugeV4.generalData.lpTokenUnderlying.toLowerCase(),
                      );
